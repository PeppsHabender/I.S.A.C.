...
object LogOverviewEmbeds {
    fun createSuccessLogsEmbed(analysis: RunAnalysis, bossData: BossData) = createEmbed(
        analysis.successLogsStr(bossData),
        CustomEmojis.SUCCESS + "__Success Logs__",
        CustomColors.GREEN_COLOR
    )

    private fun RunAnalysis.successLogsStr(bossData: BossData) = StringBuilder().apply {
        pulls
            .filter { it.success }
            .forEach {
    ...
    fun createWipeLogsEmbed(analysis: RunAnalysis, bossData: BossData) = createEmbed(
        analysis.wipeLogsStr(bossData),
        CustomEmojis.WIPES + "__Wipe Logs__",
        CustomColors.RED_COLOR
    )

    private fun RunAnalysis.wipeLogsStr(bossData: BossData) = StringBuilder().apply {
        pulls
            .filter { !it.success }
            .forEach {
                append("[")
