...
import org.inquest.utils.ignoredBosses
...
class AnalyzerService {
    ...
    fun analyze(logs: List<Pair<String, JsonLog>>): RunAnalysis = logs.sortedBy { it.second.startTime() }.let { sorted ->
        ...
        for ((link, log) in sorted) {
            downtime += if(end == null) Duration.ZERO else java.time.Duration.between(end, log.startTime()).toKotlinDuration()
            end = log.endTime()
            ...
            pulls += Pull(
                ...
                log.isCM,
                logDuration,
                (targetAlive?.finalHealth ?: 0) / (targetAlive?.totalHealth ?: 1).toDouble()
            )
            if(!log.success) {
    ...
    }.mapValues { it.value.sum() }.entries.let { players ->
        players.sumOf { it.value }
    }
    ...
    private fun addPlayerStats(base: MutableMap<String, PlayerAnalysis>, log: JsonLog) {
        ...
        players.entries.sortedByDescending { it.key }.forEachIndexed { i, (_, v) -> v.withDpsPos(i)}
    ...
    private fun JsonActorParent.JsonPlayer.fetchDps(bossId: Long?) = this.dpsTargets
        ...
        .mapNotNull { dpsTargets -> dpsTargets[0].dps
