...
class BossData {
    ...
    fun init() {
        this::class.java.classLoader.getResourceAsStream("boss_data.json")?.use { `is` ->
            this.bossData = this.objectMapper.readValue<List<IsacBoss>>(`is`).associateBy { it.eliteInsightsId }
        }
    }

    fun ignore(id: Long?) = !(this.bossData[id]?.validForTopStat ?: true)

    fun emoteFor(id: Long?, cm: Boolean) = this.bossData[id]?.emote?.let {
        if(cm) it.challenge else it.normal
    }
